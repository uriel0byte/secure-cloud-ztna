...
WireGuard is a modern VPN protocol — faster and significantly simpler than OpenVPN or IPSec. Tailscale sits on top of WireGuard and handles all the key exchange, device authentication, and mesh networking automatically through its control plane. Each device on the Tailnet gets a stable private IP in the `100.x.x.x` range regardless of physical location. This project used that private IP as the only allowed source for SSH traffic.

**Further reading:** [Tailscale How It Works](https://tailscale.com/blog/how-tailscale-works) | [WireGuard Technical Whitepaper](https://www.wireguard.com/papers/wireguard.pdf)
